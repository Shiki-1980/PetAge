...
# train_set= RegressionDataset(metadata_file, image_dire, ttxt, 'pre.txt', preprocess, txtprocess, True )
test_set1 = RegressionDataset(test_file, test_dir, tbtxt,  preprocess, txtprocess, True,'pre.txt')
test_set2 = RegressionDataset(test_file, test_dir, tbtxt, preprocess, txtprocess, False,'pre.txt')
# test_set1 = RegressionDataset(test_file, test_dir, tbtxt,  preprocess, txtprocess, True,)
# test_set2 = RegressionDataset(test_file, test_dir, tbtxt, preprocess, txtprocess, False,)
# print(len(train_set))
# train_loader = DataLoader(train_set, batch_size=128, num_workers= 8,pin_memory=True, shuffle=True)
test_loader1 = DataLoader(test_set1, batch_size=128, num_workers= 8,pin_memory=True, shuffle= False)
test_loader2 = DataLoader(test_set2, batch_size=128, num_workers= 8,pin_memory=True, shuffle= False)
...
state_dict = torch.load(f'weight/epoch_12.pth')
result1 = []
result2 = []
...
with torch.no_grad():
    test_total = 0
    test_loss = 0.0
    for images,txt, y in tqdm(test_loader1,desc="processing"):
        ...
        test_total+= y.size(0)
        result1 += preds.flatten().tolist()

    avg_loss1 = test_loss/test_total
    # test_l.append(avg_loss)
    print(f"{RED}Epoch , test Loss: {avg_loss1:.4f}{RESET}")

state_dict = torch.load(f'weight/epoch_6.pth')
result = []
model.load_state_dict(state_dict)
model.eval()
with torch.no_grad():
    test_total = 0
    test_loss = 0.0
    for images,txt, y in tqdm(test_loader2,desc="processing"):
        images = images.to(device)
        txt = txt.to(device)
        txt = txt.squeeze(dim=1)
        y = y.float().unsqueeze(1).to(device)  # shape: [batch_size, 1]
        ...
        test_loss+= (loss.item()*y.size(0))
        test_total+= y.size(0)
        result2 += preds.flatten().tolist()

    avg_loss2 = test_loss/test_total
    # test_l.append(avg_loss)
    print(f"{RED}Epoch , test Loss: {avg_loss2:.4f}{RESET}")
min = avg_loss1*len(test_set1)+avg_loss2*len(test_set2)
min = min/(len(test_set1)+len(test_set2))
print(f"{BLUE}final result is {min}{RESET}")

with open('result.txt','w') as f:
    for i in range(len(test_set1)):
        f.write(f'{os.path.basename(test_set1.image_paths[i])}\t{result1[i]}\n')
    for i in range(len(test_set2)):
        f.write(f'{os.path.basename(test_set2.image_paths[i])}\t{result2[i]}\n')
